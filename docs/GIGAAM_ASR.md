...
Образ **`worker-final-gpu`** (`docker/Dockerfile.worker.gpu`): `poetry install --with gigaam` + `install-torch.sh cuda` (группа `gigaam` в `pyproject.toml`, longform extra).
...
- Тяжёлые зависимости (torch 2.10, ~1.5 GB весов) — Poetry-группа **`gigaam`** в образе `worker-final-gpu`, не в `main`.
